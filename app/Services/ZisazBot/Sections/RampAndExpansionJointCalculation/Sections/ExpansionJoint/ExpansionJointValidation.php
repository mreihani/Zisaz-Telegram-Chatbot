...
class ExpansionJointValidation extends ExpansionJointCalculation {
    ...
    public function isNumericValidation($text, $paramType) {
        if(!is_numeric($text)) {
            ...
            $expansionJoint = new ExpansionJointBotResponse($this->telegram);

            $this->switchValidation($expansionJoint, $paramType);
