...
use PDF;
use App\Services\ZisazBot\Sections\FacadeAndFlooringMaterialCalculation\Sections\Ceramic\CeramicResult;
use App\Services\ZisazBot\Sections\FacadeAndFlooringMaterialCalculation\Sections\Ceramic\CeramicCalculation;

class CeramicBotResponse extends CeramicCalculation {
    ...
    public function __construct($telegram) {
        ...
        $this->ceramic = $this->latestAction->ceramic->first();
    }

    public function processParameterSubmission() {
        if(empty($this->ceramic->t)) {
            return $this->sendPamameterTText();
        } elseif(empty($this->ceramic->a)) {
    ...
    public function sendPamameterTText() {
        try {
            $text = 'ضخامت متوسط ملات سرامیک را بر حسب سانتی متر وارد نمایید';
    ...
    public function displayFinalResults() {
        ...
        $ceramicResult = new CeramicResult($this->telegram);

        $results = $ceramicResult->calculateCeramic();
...
⤵
        ...
        $option = array( 
            // First row
            array($this->telegram->buildInlineKeyBoardButton('⬇ دانلود پی دی اف محاسبات', '', '/ceramicdownloadresults')), 
            // Second row
            array($this->telegram->buildInlineKeyBoardButton('🔁 پروژه جدید', '', '/ceramicresetresults')), 
            // Third row
            array($this->telegram->buildInlineKeyBoardButton('🔙 بازگشت', '', '/start')), 
        );
    ...
    public function downloadResults() {
        ...
        $data = $ceramicResult->calculateCeramic();

        // Step 1: Generate the PDF content
        $pdf = PDF::loadView('facade-and-flooring-material-calculation.generatepdf-ceramic', $data);
